...
async function load(){try{const r=await fetch("../data/cf4/scene-mobile.json",{cache:"no-store"});if(!r.ok)throw 0;scene=await r.json();try{const vr=await fetch("../data/cf4/velocity-samples.json",{cache:"no-store"});if(vr.ok){const vd=await vr.json();velocitySamples=(vd.objects||[]).filter((_,i)=>i%Math.max(1,Math.ceil((vd.objects||[]).length/800))===0)}}catch{}document.querySelector("#status").textContent=(scene.review?"REVIEW FIXTURE · ":"")+(((scene.layers[0].data?.length??0)===0)?"synthetic galaxies removed":((scene.layers[0].data?.length??scene.layers[0].count??0).toLocaleString()+" galaxies"))+" · "+((scene.layers[1].data?.length??scene.layers[1].count??0).toLocaleString())+" review flows";document.querySelector("#scale").textContent="Supergalactic · "+(scene.coordinateFrame?.units||"units provisional")}catch{scene=demo;document.querySelector("#status").textContent="Preview geometry · CF4 data not built yet";document.querySelector("#scale").textContent="Supergalactic · demonstration only"}}load();
